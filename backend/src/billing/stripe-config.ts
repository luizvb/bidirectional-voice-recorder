...
export function assertVoxaPrice(price: Stripe.Price, config: VoxaStripeConfig) {
  const commercialPlan = firstVoxaCommercialPlan();
  ...
  if (price.metadata?.owner_brand !== 'netolabs' || price.metadata?.product_key !== 'voxa' || price.metadata?.package_key !== commercialPlan.key || price.metadata?.entitlement_key !== commercialPlan.entitlementKey || price.metadata?.catalog_version !== '2026-07-14' || price.metadata?.commercial_status !== commercialPlan.commercialStatus) throw new BillingConfigurationError('Voxa Stripe Price metadata is invalid.', 409, 'BILLING_CATALOG_MISMATCH');
